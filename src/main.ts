...
const handleDownload = async (url: string, pwd: string): Promise<void> => {
  ...
  return new Promise<void>((resolve, reject) => {
    const fileStream = createWriteStream(pwd);
...
const main = async () => {
  ...
  if (args.length < 1) {
    ...
    console.log(
      "this would download cancer into ./cancer and would keep on doing it indefinitely"
    );
    return;
  }
